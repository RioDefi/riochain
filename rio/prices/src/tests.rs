...
fn lock_price_call_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        let lock_price_event =
            TestEvent::prices(Event::LockPrice(BTC, Price::saturating_from_integer(5000)));
        assert!(System::events()
            .iter()
            .any(|record| record.event == lock_price_event));
        assert_eq!(
...
fn unlock_price_call_work() {
    ExtBuilder::default().build().execute_with(|| {
        ...
        LockedPrice::insert(BTC, Price::saturating_from_integer(8000));
        assert_noop!(
            PricesModule::unlock_price(Origin::signed(5), BTC),
            BadOrigin,
        );
        ...
        assert!(System::events()
            .iter()
            .any(|record| record.event == unlock_price_event));
