...
use log::info;
use sc_cli::{display_role, informant, parse_and_prepare, NoCustom, ParseAndPrepare, SharedParams};
pub use sc_cli::{error, IntoExit, VersionInfo};
use sc_service::{AbstractService, Configuration, Roles as ServiceRoles};
use tokio::runtime::Runtime;

use primitives::crypto::{set_default_ss58_version, Ss58AddressFormat};
    ...
    E: IntoExit,
{
    ...
    let opt = parse_and_prepare::<NoCustom, NoCustom, _>(&version, env!("CARGO_PKG_NAME"), args);
    match opt {
...
fn set_address_type(chain_key: &str) -> Result<(), String> {
    ...
    let v = match alternative {
        chain_spec::Alternative::Beta => Ss58AddressFormat::try_from("42").expect("must use u8"),
        _ => Ss58AddressFormat::try_from("42").expect("must use u8"),
    };
